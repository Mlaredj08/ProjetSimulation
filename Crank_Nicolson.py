...
class WaveFunction(object):
    ...
    def __init__(self, x, y, psi_0, V, dt, hbar, m, t0=0.0):
        self.x = np.array(x)
        self.y = np.array(y)
        ...
        #Construire la première matrice pour résoudre le système (A À partir de : Ax_{n+1}=Mx_{n})
        ...
        #Construire la deuxième matrice pour résoudre le système (M À partir de : Ax_{n+1}=Mx_{n})
        I = np.zeros(size)
        J = np.zeros(size)
        K = np.zeros(size, dtype=np.complex128)
